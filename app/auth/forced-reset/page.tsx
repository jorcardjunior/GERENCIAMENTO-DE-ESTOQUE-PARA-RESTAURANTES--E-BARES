...
export default function ForcedResetPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/auth/forced-reset", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, name, currentPassword, password }),
      });
        ...
        <div className="bg-surface-secondary/80 border border-border rounded-3xl p-8 shadow-2xl shadow-black/20">
          <div className="text-center mb-8">
            ...
            <h1 className="text-xl font-bold text-text tracking-tight">
              Configurar <span className="text-emerald-400">Conta</span>
            </h1>
            <p className="text-slate-400 text-sm mt-1">Defina seu email e senha para começar</p>
          </div>
