...
export default function GestaoPage() {
  ...
  const addCustomColumn = () => {
    ...
    try {
      localStorage.setItem("gestao_custom_columns", JSON.stringify(next));
    } catch {}
    setNewCustomColName("");
  };
  ...
  const deleteCustomColumn = (id: string) => {
    ...
    setVisibleColumns((prev) => {
      ...
      try {
        localStorage.setItem("gestao_visible_columns", JSON.stringify(filtered));
      } catch {}
      return filtered;
    });
  };
  ...
  const COLUMN_DEFS: {
    ...
    () => [
      ...
      { id: "status", label: "Situação", defaultVisible: true, builtin: true },
  ...
  const toggleColumn = (id: string) => {
    setVisibleColumns((prev) => {
      ...
      try {
        localStorage.setItem("gestao_visible_columns", JSON.stringify(next));
      } catch {}
      return next;
    });
  };
  ...
  const _setCustomField = (itemId: number, colId: string, value: string) => {
  ...
  const stats = useMemo(() => {
    ...
    const estoqueTotal = allItems.reduce((sum, i) => sum + Number(i.currentQuantity), 0);
  ...
  async function addCategory() {
    ...
    if (res.ok) {
      ...
      setTimeout(() => setRecentIds((prev) => prev.filter((id) => id !== data.id)), 4000);
      loadData();
    } else {
      const data = await res.json();
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 border-2 border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-[#2563eb] focus:border-transparent text-sm bg-surface"
          />
        </div>
        {isAdmin && (
        ...
        <button
          ...
          className="flex items-center gap-2 px-3 py-2.5 bg-surface text-text-secondary border-2 border-border rounded-xl hover:bg-surface-hover hover:border-[#2563eb] hover:text-brand-600 transition-all text-sm font-bold shrink-0"
            ...
            <div className="space-y-4">
              <div className="relative">
                ...
                <select
                  ...
                  className={`w-full px-3 py-2.5 border-2 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#2563eb] text-sm bg-surface ${
                    categorySuggestion ? "border-purple-300 bg-purple-50" : "border-border"
                  }`}
                >
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full px-3 py-2.5 border-2 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#2563eb] text-sm bg-surface ${
                    itemSuggestion ? "border-amber-300 bg-amber-50" : "border-border"
                  }`}
                />
                {itemSuggestion && (
                  <div className="flex items-center gap-1.5 mt-1.5">
                    <span className="text-[9px] text-amber-700 bg-amber-100 border border-amber-200 px-2 py-0.5 rounded-full">
                    ...
                    <button
                      ...
                      className="text-[9px] font-bold text-amber-700 bg-amber-200 hover:bg-amber-300 px-2 py-0.5 rounded-full transition-colors"
                    >
                      Usar este
                    </button>
                  </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2.5 border-2 border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-[#2563eb] text-sm bg-surface"
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-text-secondary mb-1">
                  ...
                  <div className="relative">
                    <select
                      ...
                      className={`w-full px-3 py-2.5 border-2 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#2563eb] text-sm bg-surface ${
                        unitSuggestion ? "border-orange-300 bg-orange-50" : "border-border"
                      }`}
                    >
                      {KNOWN_UNITS.map((u) => (
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-text-secondary mb-1">
                    Valor Un. (R$) <span className="text-text-tertiary">(opcional)</span>
                  </label>
                  <input
                    type="number"
                    step="0.01"
      ...
      {showNewCat && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          />
          <div className="relative bg-surface rounded-2xl shadow-2xl border-2 border-border w-full max-w-lg p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-black uppercase tracking-tight text-text">
                Nova Categoria
              </h2>
            ...
            <div className="relative">
              ...
              {catSuggestions.length > 0 && (
                <div className="absolute z-20 top-full left-0 right-0 mt-1 bg-surface border-2 border-border rounded-xl shadow-xl overflow-hidden">
                    ...
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 text-sm transition-colors ${
                        i === catSuggestionIndex
                          ...
                          : "hover:bg-surface-active text-text"
                      }`}
                      onClick={() => {
                        setNewCatName(s);
                        setCatSuggestions([]);
              ...
              <div className="flex gap-2.5 flex-wrap">
                {CATEGORY_COLORS.map((c) => (
                  <button
                    ...
                    className={`h-6 w-6 rounded-full transition-all duration-200 ${editCatColor === c ? "ring-2 ring-offset-2 ring-text scale-110" : "hover:scale-110"}`}
                    style={{ backgroundColor: c }}
                    title={c}
                  />
                ))}
      ...
      {allItems.length === 0 ? (
        <div className="text-center py-20 max-w-md mx-auto">
          <Package className="h-16 w-16 mx-auto mb-6 opacity-20 text-text-secondary" />
          <h2 className="text-2xl font-black text-text mb-2">Estoque vazio</h2>
          <p className="text-text-secondary mb-2">
            Crie sua primeira categoria e adicione insumos para começar.
          </p>
          {isAdmin && (
            <button
              onClick={() => setShowNewCat(true)}
              className="mt-6 px-4 py-2 bg-[#2563eb] text-white rounded-xl hover:bg-[#1d4ed8] text-sm font-bold"
            >
        ...
        <div className="space-y-8">
              ...
              <section key={cat.id} className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-bold text-text-tertiary bg-surface-tertiary px-2 py-0.5 rounded-full">
                  ...
                  {isAdmin && (
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="p-1.5 text-brand-600 hover:bg-brand-500/10 rounded-lg transition-colors"
                ...
                <div
                  className="overflow-x-auto rounded-2xl border-2 border-border bg-surface shadow-lg"
                      ...
                      <tr className="bg-surface-secondary border-b-2 border-border">
                        {COLUMN_DEFS.map((col) => {
                          ...
                          return (
                            <th
                              key={col.id}
                              className={`text-left p-4 font-bold uppercase text-[10px] tracking-widest text-text-tertiary whitespace-nowrap ${col.id === "acoes" ? "text-right" : ""}`}
                            >
                              {col.label}
                            </th>
                          );
                          ...
                          <td
                            colSpan={
                              COLUMN_DEFS.filter((c) => {
                                ...
                                return visibleColumns.includes(c.id);
                              }).length
                            }
                          ...
                          return (
                            ...
                            >
                              {visibleColumns.includes("nome") && (
                                <td className="p-4">
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                      <span
                                        className={`font-bold text-text ${isRecent ? "text-emerald-500" : ""}`}
                                      >
                                        {item.name}
                                      </span>
                                      {isRecent && (
                                        <span className="ml-2 text-[9px] font-bold text-emerald-600 bg-emerald-500/10 px-1.5 py-0.5 rounded uppercase">
                                          NOVO
                                        </span>
                                      )}
                                      {isExpiringSoon(item.expiryDate) && (
                                        <span className="ml-2 text-[9px] font-bold text-rose-500 bg-rose-500/10 px-1.5 py-0.5 rounded uppercase tracking-wider">
                                          Expira{" "}
                                          {item.expiryDate
                              ...
                              {visibleColumns.includes("estoqueAtual") && (
                                <td
                                  className={`p-4 ${qty <= 0 ? "bg-rose-500/5" : qty <= min * lowStockMultiplier ? "bg-amber-500/5" : "bg-emerald-500/5"}`}
                                >
                                  <span
                                    className={`font-black text-lg tabular-nums ${qty <= 0 ? "text-rose-500" : qty <= min * lowStockMultiplier ? "text-amber-500" : "text-emerald-500"}`}
                                  >
                              ...
                              {visibleColumns.includes("unidade") && (
                                <td className="p-4">
                                  <span className="text-[10px] font-black text-text-tertiary uppercase tracking-widest bg-surface-tertiary px-2 py-1 rounded">
                                    {item.unit}
                                  </span>
                                </td>
                              )}
                              {visibleColumns.includes("valorUnitario") && (
                                <td className="p-4">
                                  <span className="font-black tabular-nums text-sm text-text">
                              ...
                              {visibleColumns.includes("status") && (
                                <td className="p-4">
                                  <span
                                    className={`inline-block text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-full border-2 ${status.color}`}
                                  >
                                    {status.label}
                                  </span>
                                </td>
                              ...
                              {visibleColumns.includes("validade") && (
                                <td
                                  className={`p-4 ${isExpiringSoon(item.expiryDate, expiryDays) ? "bg-amber-500/10" : ""}`}
                                >
                                  <span
                                    className={`text-xs font-semibold ${isExpiringSoon(item.expiryDate, expiryDays) ? "text-amber-500" : "text-text-tertiary"}`}
                                  >
                                    {item.expiryDate
                                      ? new Date(item.expiryDate).toLocaleDateString("pt-BR")
                                      : "Não preenchido"}
                              ...
                              {customColumns
                                ...
                                .map((col) => (
                                  <td key={col.id} className="p-4">
                                    <span className="text-xs text-text-tertiary">
                                      {customFieldValues[item.id]?.[col.id] || "—"}
                                    </span>
                                  </td>
                                ))}
                              {isAdmin && visibleColumns.includes("acoes") && (
                                <td className="p-4 text-right whitespace-nowrap">
                                  <button
                                    ...
                                    className="p-1.5 text-brand-600 hover:bg-brand-500/10 rounded-lg transition-colors mr-1"
                                    title="Editar item"
                                  >
                                    <Pencil className="w-4 h-4" />
                                  </button>
          ...
          {search &&
            categories.every(
              (cat) =>
                cat.items.filter((i) => i.name.toLowerCase().includes(search.toLowerCase()))
                  .length === 0,
            ) && (
              <div className="text-center py-16 text-text-secondary">
                ...
                <button
                  onClick={() => setSearch("")}
                  className="text-brand-600 underline text-sm mt-2"
                >
                  Limpar busca
                </button>
              </div>
          ...
          categories.some(
            (cat) =>
              cat.items.filter((i) => i.name.toLowerCase().includes(search.toLowerCase())).length >
              0,
          ) ? (
            <div className="flex items-center justify-between text-xs text-text-tertiary pt-4 border-t border-border">
              ...
              <div className="flex gap-3">
                ...
                <span className="flex items-center gap-1">
                  <span className="h-2 w-2 rounded-full bg-amber-500" /> Baixo
                </span>
                <span className="flex items-center gap-1">
                  <span className="h-2 w-2 rounded-full bg-rose-500" /> Vencendo
                </span>
                <span className="flex items-center gap-1">
                    ...
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all shrink-0 ${
                          isVisible ? "bg-[#2563eb] border-[#2563eb]" : "border-border"
                        }`}
                      >
                        {isVisible && (
                          <svg
                      ...
                      {isBuiltin && (
                        <span className="text-[9px] font-bold text-text-tertiary bg-surface-tertiary px-1.5 py-0.5 rounded uppercase">
                          Padrão
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className={`text-[11px] font-bold uppercase tracking-wider px-3 py-1 rounded-lg transition-all ${
                          isVisible
                            ? "text-brand-600 bg-blue-100 hover:bg-blue-200"
                            : "text-text-tertiary bg-surface-tertiary hover:bg-surface-active"
                        }`}
                      >
            ...
            <div className="mt-6 pt-4 border-t border-border">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2.5 border-2 border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-[#2563eb] text-sm bg-surface"
                />
                <button
              ...
              <p className="text-[10px] text-text-tertiary mt-1">
                Colunas personalizadas são sempre do tipo texto.
              </p>
            </div>
          </div>
        </div>
